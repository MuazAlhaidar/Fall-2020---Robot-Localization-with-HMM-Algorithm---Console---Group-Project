...
function smoothpart( grid::Array{Array{Float64,1}}, last_grid::Array{Array{Float64,1}}, Bgrid::Array{Array{Float64,1}},  evidence::Tuple{SquareType, SquareType, SquareType, SquareType},  dir::Direction, pos::Tuple{Int64, Int64})
	...
	for i in parent_pos
		...
		# println(round(y, digits=4), "\t", tmp_pos, "\t", round(cep(grid, tmp_pos, evidence), digits=4),"\t",  drift, "\t", Bgrid[tmp_pos[1]][tmp_pos[2]])
		# println(round(y, digits=4), "\t", round(cep(grid, tmp_pos, evidence), digits=4),"\t",  drift, "\t", Bgrid[tmp_pos[1]][tmp_pos[2]])
		# println(round(y, digits=4), "\t", round(cep(grid, tmp_pos, evidence), digits=4),"\t",  drift)
		x+=y
	end
	
	println(x, "\t\t\t", last_grid[pos[1]][pos[2]], "\t\t\t",  x *last_grid[pos[1]][pos[2]])
...
# for row in Iterators.Reverse(1:6)
# 	for col in 1:5
# 		if(notblocked(grid, (row,col)))
# 			 smoothpart(tmp5,tmp3,Bgrid,(CLOSED,CLOSED,OPEN,CLOSED),NORTH, (row,col))
# 			 println()
# 		end
